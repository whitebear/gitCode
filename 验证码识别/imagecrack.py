          ...
          for n in Data.N:
              ...
              for i in range(len(l)):
                  if (n[i] == 1):
                      ...
                      if (l[i] == 1):
                          count4+=1
              d_print(count1,count2,count3,count4,count5)
          ...
          for i in range(10):
              ...
              if (min1[i] <= 3 or min2[i] <= 3):
                  ...
                  if (not flag):
                      return i
          for i in range(10):            
              if (min1[i] <= 7 or min2[i] <= 7):
                  if (abs(min1[i] - min2[i]) <= 20):
                      return i
          for i in range(10):
              if (min1[i] <= 10 or min2[i] <= 10):
                  if (abs(min1[i] - min2[i]) <= 15):
                      return i
          for i in range(10):
              if (min1[i] <= 15 or min2[i] <= 15):
                  if (abs(min1[i] - min2[i]) <= 5):
                      return i
...
def Pic_Reg(image_name=None):
          ...
          #im.show()
                          #all by pixel
          s = [7,20,33,46]          #start postion of first number
          w = 9          #width of each number
          h = 13          #end postion from top
          t = 3           #start postion of top

          num = 4
          im_new = []
          #split four numbers in the picture
          for i in range(num):
              im1 = im.crop((s[i],t,s[i]+w,t+h))
              im_new.append(im1)
          s = ""
          for k in range(num):
              l = []
...
#for ii in range(100):
#    #ii=93
#    imagename = "./image/"+str(ii)+".jpg"
#    print imagename+":" +Pic_Reg(imagename)
