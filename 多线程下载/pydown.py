...
class AxelPython(Thread, urllib.FancyURLopener):
    ...
    def run(self):
        ...
        print '\rtask %s will download from %d to %d' % (self.name, self.startpoint, self.ranges[1])
...
def paxel(url, output, blocks=6, proxies=local_proxies):
    ...
    while islive(tasks):
        ...
        show = u'\rFilesize:%d Downloaded:%dm%dk Completed:%.2f%%' % (size, downloaded/(1024*1024),(downloaded%(1024*1024))/1024, process)
